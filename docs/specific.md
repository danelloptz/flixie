...
1. **Пользователь запускает веб-приложение.**  
   Система отображает страницу авторизации/регистрации с двумя вариантами входа:  
   - Вход через логин и пароль;  
   - Вход через Google (кнопка «Использовать Google»).  

2. **Неавторизованный пользователь пытается перейти на главную страницу или любую другую защищённую страницу.**  
   Система автоматически перенаправляет его на страницу авторизации/регистрации. Доступ к любому функционалу без авторизации **запрещён**.

3. **Пользователь выбирает регистрацию через логин и пароль.**  
   Система отображает форму регистрации с полями:  
   - Логин (3–30 символов);  
   - Пароль (8–64 символа);  
   - Подтверждение пароля;  
   - Кнопка «Продолжить».  
   После ввода данных пользователь нажимает «Продолжить».

4. **Пользователь загружает аватар профиля.**  
   Система открывает окно выбора файла.  
   - Если файл превышает 5 МБ — появляется сообщение об ошибке, поле очищается.  
   - Если файл корректен — аватар отображается в виде круга, кнопка «Загрузить другую» и «Продолжить» активны.  
   Пользователь нажимает «Продолжить».

5. **Пользователь выбирает любимые жанры фильмов.**  
   - При повторном нажатии на выбранный жанр — он снимается.  
   Пользователь нажимает «Завершить».

6. **После завершения регистрации система показывает пользователю баннер о необходимости подтвердить эл. почту**  
   Пользователь переходит в настройки и вводит адрес эл. почты, нажимает на кнопку "Подтвердить"
   Все функции, кроме «Настройки профиля», **заблокированы** до подтверждения почты.

7. **Пользователь переходит по ссылке в письме для подтверждения почты.**  
   Система проверяет уникальность токена, обновляет статус аккаунта на «подтверждённый» и автоматически перенаправляет на главную страницу приложения.  
   Элементы меню «Главная», «Друзья», «Сессии», «Уведомления», «Настройки» теперь **отображаются**.

8. **Пользователь нажимает на иконку «Настройки».**  
   Система отображает страницу настроек с возможностью:  
   - Редактирования имени, фамилии, ники;  
   - Привязки новой почты (с обязательным подтверждением через письмо);  
   - Изменения аватара (с валидацией размера ≤5 МБ);  
   - Управления любимыми жанрами;  
   - Отвязки аккаунта Google (профиль сохраняется, можно войти через логин/пароль).  
   При изменении почты — система отправляет новое письмо с подтверждением.

9. **Пользователь нажимает на иконку «Создать новую сессию».**  
   Система открывает страницу создания сессии с полями:  
   - Название сессии (обязательное поле);  
   - Время жизни: часы (0–23), минуты (0–59);  
   - Чекбокс «Использовать мои жанры» — автоматически заполняет список жанров из профиля;  
   - Чекбокс «Показывать контент 18+»;  
   - Ползунки для выбора диапазона года выпуска (1900–текущий год);  
   - Выпадающий список стран производства;  
   - Кнопка «Пригласить друзей».  
   Пользователь заполняет поля и нажимает «Пригласить друзей».

10. **Пользователь приглашает друзей в сессию.**  
    Система отображает список друзей (максимум 200) с аватарами, именами и статусом дружбы.  
    - Пользователь может искать друзей по нику/имени — результаты отображаются в реальном времени.  
    - Пользователь кликает на друзей — они выделяются.  
    - Максимум 10 друзей могут быть выбраны.  
    Пользователь нажимает «Создать сессию».

11. **Система создает сессию и переводит её в активное состояние.**  
    - Пользователь попадает в интерфейс сессии — отображается карточка фильма.  
    - На экране отображается: название, год, жанры, рейтинг IMDb, постер, краткое описание.  
    - Фильмы не повторяются для одного пользователя в рамках сессии.  
    - Создатель сессии — единственный, кто может запустить её после приглашений.
12. **Пользователь взаимодействует с карточкой фильма:**  
    - **Свайп влево** → система регистрирует «Интересно», анимация свайпа, карточка уходит влево.  
    - **Свайп вправо** → система регистрирует «Не интересно», карточка уходит вправо, фильм исключён из рекомендаций.  
    - **Нажатие или свайп вверх** → карточка раскрывается полностью: показываются актёры, трейлер, подробное описание, IMDB-ссылка.  
    Все действия синхронизируются с бэкендом и передаются другим участникам сессии.

13. **Пользователь добавляет ещё одного участника в сессию (через уведомление).**  
    - Получатель получает Push- и email-уведомление:  
      > «[Имя] пригласил вас в сессию “[Название сессии]” (длится X часов).»  
      С кнопками «Принять» / «Отклонить».  
    - При нажатии «Принять» — пользователь попадает в интерфейс сессии.  
    - При нажатии «Отклонить» — уведомление исчезает, статус обновляется.  
    - Участник становится частью сессии — общее количество участников не превышает 10.

14. **Все участники сессии проголосовали за один и тот же фильм.**  
    Система определяет **Match** и:  
    - Останавливает показ новых карточек;  
    - Отображает экран мэтча: крупный постер фильма, название, рейтинг, надпись «Вы нашли идеальный фильм!»;  
    - Отправляет Push- и email-уведомления всем участникам сессии.  
    - Match **не возникает**, если хотя бы один участник не проголосовал.

15. **Пользователь открывает страницу «Уведомления».**  
    Система отображает список уведомлений в хронологическом порядке:  
    - О мэтче (с названием фильма и временем события);  
    - О запросе в друзья (с аватаром и именем отправителя);  
    - О приглашении в сессию (с названием сессии и именем пригласившего).  
    - Каждое уведомление содержит кнопки «Принять» / «Отклонить» (где применимо).  
    - При нажатии на уведомление о мэтче — открывается карточка фильма в режиме просмотра.  
    - При нажатии на уведомление о запросе в друзья — появляются кнопки «Принять» / «Отклонить».  
    - После принятия/отклонения — уведомление помечается как «прочитанное» и исчезает из списка.  
    - При открытии страницы уведомлений — все непрочитанные помечаются как прочитанные.

16. **Пользователь открывает страницу «Друзья».**  
    Система отображает:  
    - Список всех друзей с аватарами, именами и статусом «друг»;  
    - Кнопку «Добавить друга» — переход на поиск;  
    - Поле поиска по нику/имени — результаты отображаются при вводе ≥4 символов.  
    - При поиске незарегистрированного или несуществующего пользователя — выводится «Пользователь не найден».  
    - При добавлении нового друга — отправляется запрос, который отображается в уведомлениях получателя.  
    - Пользователь не может добавить более 200 друзей — при попытке добавить 201-го — появляется ошибка.

17. **Пользователь создаёт новую сессию, когда уже есть 5 активных.**  
    Система блокирует действие и отображает сообщение:  
    > «Вы достигли лимита в 5 активных сессий. Завершите одну из них, чтобы создать новую.»

18. **Пользователь пытается добавить в сессию 11-го участника.**  
    Система блокирует добавление и отображает ошибку:  
    > «Максимальное количество участников в сессии — 10.»

19. **Пользователь выходит из аккаунта (logout).**  
    Система:  
    - Деактивирует access- и refresh-токены на сервере;  
    - Удаляет токены из localStorage на клиенте;  
    - Перенаправляет на страницу авторизации.  
    После выхода — повторный вход требует ввода логина/пароля или OAuth.

20. **Пользователь меняет пароль.**  
    Система:  
    - Запрашивает текущий пароль и новый (8–64 символа);  
    - После сохранения — деактивирует все существующие токены;  
    - Пользователь должен заново авторизоваться.  
    Пароли хранятся в базе только в хешированном виде (bcrypt).
21. **Пользователь использует OAuth (Google) для входа.**  
    - Первый вход: система создаёт аккаунт, запрашивает выбор жанров, отправляет письмо на подтверждение почты.  
    - Повторный вход: система привязывает аккаунт Google к существующему email, автоматически перенаправляет на главную страницу.  
    - При попытке войти через Google с email, не связанным с аккаунтом — создаётся новый аккаунт.

22. **Гость (не подтверждённый email) пытается открыть страницу сессий, друзей или уведомлений.**  
    Система:  
    - Блокирует доступ;  
    - Отображает модальное окно:  
      > «Для доступа к этой функции подтвердите вашу электронную почту.»  
      + Кнопка «Отправить письмо снова».  
    - Пользователь может отправить повторное письмо — оно приходит с новой ссылкой.

23. **Пользователь проверяет работу PWA.**  
    - Приложение установлено на домашний экран iOS/Android — отображается корректная иконка и название.  
    - При отключении интернета — приложение открывается, и сообщает пользователю о необходимости подключения к интернету.  
    - Карточки фильмов и аватары подгружаются при наличии сети (lazy load).

24. **Пользователь проверяет адаптивность интерфейса на устройстве с шириной 320px.**  
    - Все элементы (карточки, свайпы, меню, кнопки) корректно отображаются и работают.  
    - Свайпы плавные, реакция на касание мгновенная.  
    - Текст не обрезается, не переносится некорректно, нет overflow.

25. **Администратор проверяет API через Swagger UI.**  
    Все эндпоинты (auth, sessions, friends, notifications, settings) доступны, документированы, содержат примеры запросов и ответов.  
    - Без JWT-токена — все запросы возвращают 401 Unauthorized.  
    - Некорректные данные (пустой email, неверный ID) — возвращают 422 Unprocessable Entity.  
    - Все запросы логируются.

26. **Система проверяется на совместимость с браузерами:** 
    - Google Chrome — работает корректно.  
    - Yandex Browser — работает корректно.  
    - Safari (iOS) — работает корректно, PWA установлена.  
    - Microsoft Edge — работает корректно.  
    - Firefox — работает корректно (опционально).  
    Все тексты и интерфейс — **на русском языке**, нет английских строк.

27. **Проверяется доставка уведомлений:**  
    - Push-уведомления приходят на мобильные устройства при:  
      - Match;  
      - Приглашении в друзья;  
      - Присоединении к сессии.  
    - Email-уведомления приходят при:  
      - Регистрации (ссылка подтверждения);  
      - Смене почты (новая ссылка);  
      - Match (ссылка на фильм).  
    - Нет дублирования уведомлений — одно событие = одно уведомление.

28. **Проверяется работа CI/CD (GitHub Actions):**  
    - Автоматический запуск тестов при пуше в main.  
    - pytest покрывает >80% ключевых эндпоинтов (регистрация, сессии, друзья, уведомления).  
    - Jest покрывает основные компоненты Vue.js (авторизация, карточка фильма, интерфейс сессии).  
    - Деплой проходит без ошибок.

29. **Проверяется stateless-архитектура:**  
    - Все данные сессий, голосов, друзей, уведомлений хранятся только в PostgreSQL.  
    - Сервер не хранит сессионные данные в памяти.  
    - JWT-токены валидируются по подписи и сроку жизни — не хранятся на сервере.